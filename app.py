...
def google_login():
    ...
    if not GOOGLE_OAUTH_AVAILABLE:
        flash('Flask-Dance not available. Please install it first.', 'warning')
        return redirect(url_for('login'))
    
    if not google_bp:
        flash('Google OAuth is not configured. Please set your Google OAuth credentials.', 'warning')
        return redirect(url_for('login'))

    # Try to access Google OAuth login
    try:
        if not google.authorized:
            return redirect(url_for("google.login"))
        else:
            flash('You are already logged in with Google!', 'info')
            return redirect(url_for('index'))
    except Exception as e:
...
def oauth_debug():
    ...
    """,
                                  ...
                                  google_login_url=url_for("google_login") if google_bp else "#")
...
def debug_endpoints():
    ...
    # Filter Google-related endpoints
    google_endpoints = [ep for ep in endpoints if 'google' in ep['endpoint'].lower()]
    
    return jsonify({
        'google_endpoints': google_endpoints,
        'google_bp_exists': google_bp is not None,
        'google_oauth_available': GOOGLE_OAUTH_AVAILABLE,
        'client_id_set': bool(app.config.get('GOOGLE_OAUTH_CLIENT_ID')),
        'client_secret_set': bool(app.config.get('GOOGLE_OAUTH_CLIENT_SECRET'))
    })
